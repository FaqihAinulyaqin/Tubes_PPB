...
class _HalamanUtamaState extends State<HalamanUtama> {
  ...
  Future<void> _getProduk() async {
    try {
      final respon = await http.get(Uri.parse('$apiUrl/api/produk/getProduk'));
  ...
  Future<void> _getCategory() async {
    try {
      final respon =
          await http.get(Uri.parse('$apiUrl/api/produk/getCategory'));
      if (respon.statusCode == 200) {
        final data = jsonDecode(respon.body);
        setState(() {
          categories =
  ...
  Future<void> _getProdukByCategory(String category) async {
    ...
    try {
      final respon =
          await http.get(Uri.parse('$apiUrl/api/produk/getProduk/$category'));
      if (respon.statusCode == 200) {
        final data = jsonDecode(respon.body);
        setState(() {
          products = List<Map<String, dynamic>>.from(data['data']);
